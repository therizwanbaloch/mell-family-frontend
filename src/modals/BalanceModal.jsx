...
import TournamentsModal from "./TournamentInfoModal";
...
const BalanceModal = ({ isOpen, onClose }) => {
  ...
  return (
    <>
        ...
        >
          ...
          <div className="flex mx-3 mb-1 rounded-xl overflow-hidden shrink-0" style={{ background: 'rgba(0,0,0,0.2)' }}>
            <button
              ...
              className="flex-1 py-2 font-black text-xs transition-all"
          ...
          <div className="flex-1 flex flex-col px-3 pb-3 overflow-hidden">
            ...
            {tab === 'balance' && (
              <div className="flex flex-col gap-2 h-full">
                ...
                <div>
                  ...
                  {/* Amount + bigger icon — change 1 & 3 */}
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="font-black text-black" style={{ fontSize: 'clamp(30px,9vw,46px)' }}>
                      {isUSDT ? '32.24' : '34508'}
                    </span>
                    <img
                      src={isUSDT ? imgUSDTCoin : imgFishki}
                      alt={isUSDT ? 'USDT' : 'Фишки'}
                  ...
                  <div className="flex items-center justify-between mt-0.5">
                    <p className="text-white font-bold text-xs">
                      {isUSDT ? 'Осталось 67.76$ до вывода' : 'Вывод доступен!'}
                    </p>
                    <div
                      className="flex items-center gap-1 px-2 py-0.5 rounded-full"
                      style={{ background: 'linear-gradient(180deg,#6ecf20,#4a9a10)', border: '1.5px solid #3a7a08' }}
                    >
                      <span className="text-black font-black text-xs">{isUSDT ? '100' : '5000'}</span>
                      <img
                        src={imgGoalBadge}
                        alt="goal"
                ...
                {/* Progress bar — change 2: usdt icon sits on top at progress edge */}
                <div className="relative">
                  <div
                    className="rounded-full overflow-hidden flex"
                    style={{ height: '16px', border: '2px solid rgba(0,0,0,0.3)' }}
                  >
                  ...
                  {/* Icon pinned to the progress edge */}
                  <img
                    src={isUSDT ? imgUSDTCoin : imgFishki}
                    alt="progress icon"
                    style={{
                      position: 'absolute',
                      left: isUSDT ? 'calc(32% - 12px)' : 'calc(100% - 12px)',
                      top: '50%',
                ...
                {/* How to withdraw toggle */}
                <button
                  onClick={() => setShowHowTo(!showHowTo)}
                  className="w-full py-2 rounded-xl font-black text-white text-xs active:scale-95 transition-transform"
                ...
                {/* Withdraw button */}
                <button
                  className="w-full py-3 rounded-2xl font-black text-black text-base active:scale-95 transition-transform mt-auto"
            ...
            {tab === 'howto' && (
              <div className="flex flex-col gap-2 h-full justify-center">
                {[
                  { img: imgSlots,      title: 'Слоты',   desc: 'Шанс на выигрыш USDT, монеты и Casino фишки',             btn: 'Слот',   action: () => { onClose(); navigate('/page24'); } },
                  { img: imgTournament, title: 'Турниры', desc: 'Бери участие в турнире и получи шанс выиграть много USDT', btn: 'Турнир', action: () => { onClose(); setShowTournaments(true); } },
                  { img: imgBoxes,      title: 'Боксы',   desc: 'Покупай боксы и забирай свои USDT, фишки, монеты.',        btn: 'Боксы',  action: () => { onClose(); navigate('/page20'); } },
                ].map((item, i) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 px-3 pt-3 pb-2">
                      {/* Icon: w5 h6 canva units — keep aspect ratio, ~44px wide 53px tall */}
                      <img
                        src={item.img}
                        alt={item.title}
                        style={{ width: '44px', height: '53px', objectFit: 'contain', flexShrink: 0 }}
                      />
                      <div className="flex-1 min-w-0">
                        <p className="text-white font-black leading-tight" style={{ fontSize: 'clamp(14px,4vw,17px)' }}>
                          {item.title}
                        </p>
                        <p className="text-white/70 leading-snug mt-0.5" style={{ fontSize: 'clamp(11px,2.8vw,13px)' }}>
                          {item.desc}
                        </p>
                      </div>
                    </div>

                    {/* Button: new line, 70% width, centered */}
                    <div className="flex justify-center pb-2.5">
                      <button
                        onClick={item.action}
                        className="rounded-xl font-black text-black active:scale-95 transition-transform"
                        style={{
                          width: '70%',
                          padding: '7px 0',
                          fontSize: 'clamp(12px,3.2vw,14px)',
                          background: 'linear-gradient(180deg,#f0c020,#d4a000)',
      ...
      <TournamentsModal isOpen={showTournaments} onClose={() => setShowTournaments(false)} />
    </>
  );
};
